		...
		<div class="col-md">
			<div class="product-item-detail-slider-container" id="<?=$itemIds['BIG_SLIDER_ID']?>">
				...
				<?=($arParams['IMAGE_RESOLUTION'] === '1by1' ? 'product-item-detail-slider-block-square' : '')?>"
                    ...
                    <div style="z-index: 120;" class="product-item-label-text <?=$labelPositionClass?>" ?>
                        <?
                        if ($arResult['PROPERTIES']['LABEL_REF']['VALUE']) {
                          $numLabel = 1;
                          foreach ($arResult['PROPERTIES']['LABEL_REF']['VALUE'] as $label) {
                       ?>
                          <span style="position: absolute; z-index: 120; top: <?= 30 * $numLabel; ?>px; left: 20px; background-color: rgb(<?= $arResult['HLB_LABELS'][$label]['UF_COLOR_RGB'] ?>);">
					        <a href="<?= $arResult['UF_LINK_LABEL'][$label]['UF_LINK_LABEL'] ?>"
                               style="text-decoration: none; color: #ffffff"><?= $arResult['HLB_LABELS'][$label]['UF_NAME_LABEL'] ?></a>
			              </span>
                          <?
                          $numLabel++;
                          }
