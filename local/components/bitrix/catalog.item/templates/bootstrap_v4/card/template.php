...
<div class="product-item">
    <div style="z-index: 120;" class="product-item-label-text product-item-label-small">
      <?
      if ($item['PROPERTIES']['LABEL_REF']['VALUE']) {
        ...
        foreach ($item['PROPERTIES']['LABEL_REF']['VALUE'] as $label) {
          ...
          ?>
            <span style="position: absolute; top: <?= 30 * $numLabel; ?>px; left: 20px; background-color: rgb(<?= $arParams['HLB_LABELS'][$label]['UF_COLOR_RGB'] ?>);">
					<a href="<?= $arParams['UF_LINK_LABEL'][$label]['UF_LINK_LABEL'] ?>"
                       style="text-decoration: none; color: #ffffff"><?= $arParams['HLB_LABELS'][$label]['UF_NAME_LABEL'] ?></a>
			</span>
          <?
        $numLabel++;
        }
